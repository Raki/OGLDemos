...
void setupScene();
void setupCSGMeshV4();
void tranformCSGMesh(std::shared_ptr<InputMesh> mesh, glm::mat4 tMat,bool updateBase=false);
...
{
    if (mesh->vcBaseArray.size() == 0)
    {
        mesh->vcBaseArray.assign(mesh->vertexCoordsArray.begin(), mesh->vertexCoordsArray.end());
    }

    std::vector<double>& vData = mesh->vcBaseArray;

    for (size_t i=0;i<vData.size();i+=3)
    {
        glm::vec4 v = glm::vec4(vData.at(i), vData.at(i+1), vData.at(i+2),1.0f);
        v = tMat*v;
        mesh->vertexCoordsArray.at(i) = v.x;
        mesh->vertexCoordsArray.at(i + 1) = v.y;
        mesh->vertexCoordsArray.at(i + 2) = v.z;
    }
