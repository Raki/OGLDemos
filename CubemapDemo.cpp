...
std::shared_ptr<Texture2D> diffuseTex, specularTex, cubeMapTex;
...
void initGL()
{
    ...
    cMapProgram = std::make_shared<GlslProgram>(
        Utility::readFileContents("shaders/vCubemap.glsl"),
        Utility::readFileContents("shaders/fCubemap.glsl")
        );
    assert(cMapProgram->programID);
    ...
    cubeMapTex->texture = GLUtility::makeCubeMap(faces);
}
...
void renderFrame()
{
    ...
    cMapProgram->bind();
    cMapProgram->setMat4f("view", glm::mat4(glm::mat3(camera->viewMat)));
    cMapProgram->setMat4f("proj", projectionMat);
    cMapProgram->setInt("sampler", 0);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP,cubeMapTex->texture);
    cMapProgram->bindAllUniforms();
    skyBox->draw();
    cMapProgram->unbind();
    glDepthFunc(GL_LESS);
