...
std::shared_ptr<GlslProgram> basicProgram,geomLightProgram;
std::vector<std::shared_ptr<Mesh>> scenObjects,defaultMatObjs,geomShaderObjs;
std::shared_ptr<Mesh> lBox;
...
void initGL()
{
    ...
    geomLightProgram = std::make_shared<GlslProgram>(
        Utility::readFileContents("shaders/gs/vMatSpotLight.glsl"),
        Utility::readFileContents("shaders/gs/gMatSpotLight.glsl"),
        Utility::readFileContents("shaders/gs/fMatSpotLight.glsl")
        );
...
void setupScene()
{
    ...
    //auto box = GLUtility::getCube(3, 3, 3);
    //scenObjects.push_back(box);

    auto hSphere = GLUtility::getHemiSphere(2, 2, 18, 18);
    hSphere->tMatrix = glm::rotate(glm::mat4(1), glm::radians(-90.0f), GLUtility::X_AXIS);
    scenObjects.push_back(hSphere);
...
void renderFrame()
{
    ...
    geomLightProgram->bind();

    geomLightProgram->setMat4f("view", camera->viewMat);
    geomLightProgram->setMat4f("proj", projectionMat);

    geomLightProgram->setVec3f("viewPos", camera->eye);

    geomLightProgram->setInt("material.diffuseSampler", 0);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, diffuseTex->texture);
    geomLightProgram->setInt("material.specularSampler", 1);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, specularTex->texture);

    geomLightProgram->setFloat("material.shininess", 64.0f);

    auto lightColor = Color::white;
    glm::vec3 diffuseColor = lightColor * glm::vec3(0.5f); // decrease the influence
    glm::vec3 ambientColor = diffuseColor * glm::vec3(0.2f); // low influence
    geomLightProgram->setVec3f("light.position", light.position);
    geomLightProgram->setVec3f("light.ambient", light.ambient);
    geomLightProgram->setVec3f("light.diffuse", light.diffuse);
    geomLightProgram->setVec3f("light.specular", light.specular);

    geomLightProgram->setFloat("light.constant", light.constant);
    geomLightProgram->setFloat("light.linear", light.linear);
    geomLightProgram->setFloat("light.quadratic", light.quadratic);

    geomLightProgram->setVec3f("light.direction", glm::vec3(0, -1, 0));
    geomLightProgram->setFloat("light.cutOff", glm::cos(glm::radians(light.cutOff)));
    geomLightProgram->setFloat("light.outerCutOff", glm::cos(glm::radians(light.outerCutOff)));

    geomLightProgram->setFloat("nDist", tLength);
    ...
    for (const auto &obj : scenObjects)
    {
        ...
        geomLightProgram->setMat4f("model", mv);
        geomLightProgram->setMat4f("nrmlMat", nrmlMat);

        geomLightProgram->bindAllUniforms();
