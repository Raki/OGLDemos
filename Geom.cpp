...
void setupScene()
{
    ...
    {
        auto v = glm::vec3(i, 0, 0);
        for (int j = 0; j < 50; j++)
        {
            auto h = rand() % 10;
            auto dir = glm::vec3(0, h/100.0f, 1);
            auto vec = v + (j * 0.1f) * dir;
            posArr.push_back(vec);
        }
    }
    for (int i = 0; i < rows-1; i++)
    {
        ...
        {
            unsigned int v0 = (i * 50) + j;
            unsigned int v1 = (i * 50) + j+1;
            unsigned int v2 = ((i+1) * 50) + j;
            unsigned int v3 = ((i + 1) * 50) + j+1;
            //v0  v2

            //v1  v3
            iData2.push_back(v1); iData2.push_back(v3); iData2.push_back(v2);
            iData2.push_back(v1); iData2.push_back(v2); iData2.push_back(v0);
        }
    }
    std::vector<glm::vec3> nArr(posArr.size());
    for (size_t i = 0; i < iData2.size(); i+=3)
    {
        auto v1 = posArr.at(iData2.at(i));
        auto v2 = posArr.at(iData2.at(i+1));
        auto v3 = posArr.at(iData2.at(i+2));
        auto n = glm::normalize(GLUtility::getNormal(v1,v2,v3));
        nArr.at(iData2.at(i + 0))+=n;
        nArr.at(iData2.at(i + 0)) /= 2.0f;
        nArr.at(iData2.at(i + 1))+=n;
        nArr.at(iData2.at(i + 1)) /= 2.0f;
        nArr.at(iData2.at(i + 2))+=n;
        nArr.at(iData2.at(i + 2)) /= 2.0f;
    }
    for (size_t i = 0; i < posArr.size(); i++ )
    {
        auto v = posArr.at(i);
        auto n = nArr.at(i);
        vData2.push_back({v,n,Color::cream});
    }
