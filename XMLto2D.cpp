...
void setupCamera();
void setupScene();
std::shared_ptr<GLUtility::Mesh> objectsFromXml(std::string path);
...
void setupScene()
{
    ...
    light.position = glm::vec3(0, 1, 400);
    light.ambient = glm::vec3(0.2f);
    light.diffuse = glm::vec3(0.176f);
    ...
    vData2.push_back({glm::vec3(-1,-1,0),glm::vec3(0,0,1),Color::red});
    vData2.push_back({ glm::vec3(1,-1,0),glm::vec3(0,0,1),Color::red });
    vData2.push_back({ glm::vec3(1,1,0),glm::vec3(0,0,1),Color::red });
    vData2.push_back({ glm::vec3(-1,1,0),glm::vec3(0,0,1),Color::red });
    ...
    auto mesh = objectsFromXml("assets/xml/grid.xml");
   
    //auto mesh = std::make_shared<GLUtility::Mesh>(vData2, iData2);
...
std::shared_ptr<GLUtility::Mesh> objectsFromXml(std::string path)
{
    auto content = Utility::readFileContents(path);
    rapidxml::xml_document<> *gridXML = new rapidxml::xml_document<>();
    gridXML->parse<rapidxml::parse_no_utf8>(content.data());

    rapidxml::xml_node<>* gridsNode = gridXML->first_node("RWFrame")->first_node("frame")->first_node("grids");

    std::unordered_map<std::string, rapidxml::xml_node<>*> grids;

    for (rapidxml::xml_node<>* child = gridsNode->first_node("grid");child;child=child->next_sibling("grid"))
    {
        auto childAttribute = child->first_attribute("gridIndex");
        grids[childAttribute->value()] = child;
    }

    fmt::print("Total grids : {}",grids.size());

    auto grid = grids["13002"];

    std::vector<glm::vec3> vArr;
    std::vector<unsigned int> iData3;
    float gHeight = 0;
    float gWidth = 0;
    if (grid != NULL)
    {
        ...
        std::vector<glm::vec3> rowPos;
        rowPos.push_back(glm::vec3(0));
        auto row = grid->first_node("row");
        
        for (;row;row=row->next_sibling("row"))
        {
            auto hStr = row->first_attribute("h")->value();
            float height = std::atof(hStr);
            gHeight += height;
            rowPos.push_back(glm::vec3(0, gHeight, 0));
            rows++;
        }
        vArr.insert(vArr.end(), rowPos.begin(), rowPos.end());

        auto col = grid->first_node("col");
        
        for (; col; col= col->next_sibling("col"))
        {
            auto wStr = col->first_attribute("w")->value();
            float width = std::atof(wStr);
            //rowPos.push_back(glm::vec3(0, height, 0));
            gWidth += width;
            for (auto v : rowPos)
            {
                auto vNew = glm::vec3(gWidth,v.y,v.z);
                vArr.push_back(vNew);
            }
            cols++;
        }
        ...
        for (decltype(cols) i=0;i<cols;i++)
        {
            for (decltype(rows) j = 0; j < rows; j++)
            {
                ...
                /* i1   i3*/
                auto i0 = (i * (rows+1)) + j;
                auto i1 = (i * (rows+1)) + j + 1;
                auto i2 = ((i + 1) * (rows+1)) + j;
                auto i3 = ((i + 1) * (rows+1)) + j + 1;
                //iData3.push_back(i1); iData3.push_back(i3); iData3.push_back(i2);
                //iData3.push_back(i1); iData3.push_back(i2); iData3.push_back(i0);
                iData3.push_back(i1); iData3.push_back(i3); 
                iData3.push_back(i3); iData3.push_back(i2);
                iData3.push_back(i2); iData3.push_back(i0);
                iData3.push_back(i0); iData3.push_back(i1);
            }
    ...
    glm::vec3 gOrigin = glm::vec3(-gWidth/2,-gHeight/2,0);
    //glm::vec3 gOrigin = glm::vec3(-5, -5, 0);
    for (auto v : vArr)
    {
        v += gOrigin;
        vData.push_back({v,glm::vec3(0,0,1),Color::cream});
    }
    auto mesh = std::make_shared<GLUtility::Mesh>(vData, iData3);
    mesh->drawCommand = GL_LINES;
